...
export default function EditHod({ route }) {
  let { hod } = route.params;
  ...
  const [imageUri, setImageUri] = useState(hod?.avtar);
  ...
  const handleEditHod = async () => {
    ...
    const { name, hodId, department,email,phone,address } = formData;

    // Check if all required fields are filled
    if (!name || !hodId || !department || !imageUri||!email || !phone || !address) {
    ...
    try {
      ...
      const formDataWithImage = { ...formData, imageUri: newAvtar, newPublicId };

      // Add HOD with image to the backend
      const apiURL = `http://${IpAddress}:5000/api/hod/edit/${hod?._id}`;
    ...
    <>
      <Block style={{ backgroundColor: 'white',flex:1 }}>
        <Appbar.Header elevated style={{ backgroundColor: 'white' }}>
          ...
          <Appbar.Content title="Edit Hod" titleStyle={{ fontSize: 18, fontFamily: 'PoppinsBold' }} />
        </Appbar.Header>
        <ScrollView
          ...
          <View style={styles.container}>
            ...
            {spinner ? (
              ...
              : (
                <ArButton style={{ width: '100%', marginTop: 20 }} onPress={handleEditHod}>
