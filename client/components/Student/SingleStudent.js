...
export default function SingleStudent({ navigation, route }) {
  ...
  let { setCount, count, allhod, role } = useContext(MyContext);
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: "Delete",
          onPress: async () => {
            const apiURL = `http://${IpAddress}:5000/api/student/updateStatus/${id}`;
            try {
              const res = await axios.post(apiURL,{status:'inactive'});
              if (res.data.success) {
                setCount(count + 1);
                navigation.goBack();
              } else {
    ...
    <>
      <Block style={{ backgroundColor: 'white', flex: 1 }}>
        ...
        <ScrollView
          ...
          <View style={{ backgroundColor: 'white', elevation: 1, margin: 15, borderRadius: 15 }}>
            <Block flex style={styles.profileCard}>
              <Block middle style={styles.avatarContainer}>
                <Image
                  source={{ uri: student?.avtar }}
                  style={styles.avatar}
                />
              </Block>
            </Block>
            ...
            <Block >
              ...
              <Block middle style={{ marginTop: 15, marginBottom: 5, padding: 2, marginHorizontal: 15, borderWidth: 0.5, borderRadius: 15, padding: 8 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-around', width: '100%', }}>
                  ...
                  <View>
                    <Text style={{ fontFamily: 'PoppinsBold', textAlign: 'center' }}>Student added on</Text>
            ...
            >
              {role == "Company" ? (
                ...
                student?.status=="active"&&(
                  <ArButton
                  style={{ borderColor: "red", backgroundColor: 'white', borderWidth: 0.5 }}
                  onPress={HandleDeleteStudent}
                // style={{ borderColor: argonTheme.COLORS.INFO ,backgroundColor:'white',borderWidth:0.5}}
                >
                  <Text style={{ fontFamily: 'PoppinsMedium',fontSize:16 }}>DELETE</Text>
                </ArButton>
                )
                )
              )}
